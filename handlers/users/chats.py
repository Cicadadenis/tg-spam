...
async def pusk_start(call: CallbackQuery):
    ...
    while i <= x:
        try:
            ...
            while q <= z:
                if m == mx:
                    await call.message.answer("Лимит 40 смс на один акаунт")
                    i = i + 1
                    mx = mx + 40
                    break
                try:
                    ...
                    await client.send_message(ff[q][:-1], sms, parse_mode="HTML")
                    time.sleep(ti)
                    await client.disconnect()
                    t = t + 1
                    m = m + 1
                    await call.message.edit_text(
                        f"Рассылка спама с акаунта №{i}\n"
                        f"Отправлено смс пользователю {ff[q][:-1]}\n"
                        f"Всего отправлено смс {t}"
                        )
                    q = q + 1
                except:break
            i = i + 1
    ...
    await call.message.answer("Рассылка Спама Успешно Завершена !")
